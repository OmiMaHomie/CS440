...
\begin{document}
    ...
    \begin{answercols}
        ...
        \\
        Let $n_i$ be the frquency of the $i$-th unique point, so the total dataset size is $N = \sigma_{i=1}^{M}n_i$. &
        A dataset with duplicates can be characterized by the counts $n_i$ of its unique points. \\
        \\
        Stanard MSE loss over the full dataset: $L(\theta) = \frac{1}{2N} \sum_{j=1}^{N}(y^{(j)} - f_{\theta}(x^{(j)}))^2$. &
    ...
    \begin{answercols}
        ...
        \\
        We define a function $p(x, y)$ over $X \times R$ as:
        $p(x, y) = \sum_{j:(x^{(j)}, y^{(j)}) = (x, y)} r^{(j)}$,
        and $p(x, y) = 0$ if $(x, y)$ is not in the dataset. &
        $p(x, y)$ aggregates the total weight assigned to all data points with value $(x, y)$. For a point not in the dataset, its value weight is simply 0. \\
        \\
        ...
        \\
        Since the ataset is finite, only finitely many terms in this sume are non-zero. &
        Ensures the sum is well-defined and compuitationally tractable, even if $X$ ens up being infinite.
    \end{answercols}
    ...
    \begin{answercols}
        ...
        In $L(\theta) = \frac{1}{2N} \sum_{i=1}^{N} r^{(i)}(y^{(i)} - f_{\theta}(x^{(i)}))^2$, whene $r^{(i)} = 0$, the entire term for that data point is zero, regardless of the model's error. \\
        \\
        ...
        \\
        The moel provides no guarantees about performace on 0-weight points. &
        The optimization process is uncertain to these points. The solution is determined entirely by points with $r^{(i)} > 0$. Performance on 0-weight points epends on inductive bias and correlation with trained points. \\
        \\
    ...
    \begin{answercols}
        Firstly, Bellman's principle of optimality would fail, as optimal policy from any state depends on rewards already observed. &
        Is is a DP issue, due to the non-stationary preferences shown from the previous proof. \\
        \\
        We can define an augmented state space $s' = (s, m)$ where $m$ tracks maximum rewards seen so far. &
        Do state space expansion, making the problem Markovian again. \\
        \\
        A policy that maximizes the probability of achieve the highest possible rewars can be defined. &
        Defining a Vlaue Function, as this is a well-defined optimization problem. \\
        \\
        $V(s, m) = \max_{a} \sum_{s'} T(s' | s, a) 1_{r \geq m} + V(s', \max(m, r))$. &
        It is complex, but matematically it works out as a Bellman equation. \\
        \\
        It is possible to define utility as indicator $U(\tau) = 1_{\text{max reward in } \tau \geq \text{threshold}}$ and maximize the expected utility. &
        Because of this possibility, we could yield a coherent objective. \\
        \\
        However, this requires knowing the maximum achievable reward in advance OR solving a complex multi-objective problem. &
        The optimization becomes much harder than a standard MDP.
    \end{answercols}
