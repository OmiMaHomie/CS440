...
\begin{document}
    ...
    \\
    Let $D$ be the dataset with $n$ positive examples $(+1)$ and $n$ negative exmaples $(-1)$ such that $|D| = 2n$. \\
    The question tells us that a leave-one-out cross-validation is performed, such essentially does the following: \\
    \\
    - For each fold, remove 1 exmaple $e$ \\
    - Train a majority classifier on the remaining $2n - 1$ exmaples \\
    - Predict the class of $e$ \\
    \\
    The majority classifier predicts the class with more examples in the training set; Since the total amount of classes in this validation is $2n - 1$, ties are impossible since the dataset is odd. \\
    \\
    \begin{answercols} 
        Let $P = {+1} \times n, N = {-1} \times n$.  &
        Defining the dataset, with $n$ positives, negatives. \\
        \\
        LOOCV trains on $D \setminus e$ such that $|D \setminus e| = 2n - 1$. &
        Defining the LOOCV procedure.
    \end{answercols}
    ...
    \begin{answercols}
        Training set: $(n - 1)$ positives, $n$ negatives. &
    ...
    \noindent\textbf{Case 2: $e \in N$} (The left-out exmaple is negative) 
    \begin{answercols}
        Training set: $(n - 1)$ negatives, $n$ positives. &
    ...
    \noindent Looking at this result, it becomes clear that the reason why there's a 0\% accuracy in the LOOCV is being the training set's majority class is always the opposite of the left-out exmaple's true class. This is due to the initial perfect balance of the dataset (since we initally have both $n$ amount of possitve, negative classes).
    \begin{answercols}
        Both cases, prediction is wrong. \\
        \\
        $\forall e \in D, \text{majorit classifier is wrong on } e.$ &
        Holds for all exmaples. \\
        \\
    ...
    \noindent\textbf{Note:} this is a \textbf{proof} question, meaning you must follow formal proof structure (see the examples of piazza for guidance).\\
\\
    Let there be $K$ independent binary classifiers, each with error probablity $\epsilon$. Let the true label for a given test point be fixed (so class 1 without loss of generality). We can define the random variable $X$ as the num. of classifiers that predict incorrectly. Then, $X ~ \text{Binomial}(K, \epsilon)$, so: \\
    \\
    \begin{equation*}
        P(X = m) = 
        \begin{pmatrix}
            ...
            m
        \end{pmatrix}
        \epsilon^{m}(1 - \epsilon)^{K - m}, m = 0, 1, \ldots, K.
    \end{equation*}
    \\
    The predicted class will be the one recieving more than half of the votes (majority voting). \\

    \begin{answercols}
        $X ~ \text{Bin}(K, \epsilon)$. &
        Num. of wronog classifiers is binomial. \\
        \\
        Ensemble wrong if majority classifiers wrong. &
        Defining the majority voting error.
    \end{answercols}
    ...
    \begin{answercols}
        ...
        \\
        \begin{equation*}
            E(K, \epsilon) = \sum_{m = t + 1}^{K} 
            \begin{pmatrix}
                K \\
                m
            \end{pmatrix}
            \epsilon^{m}(1 - \epsilon)^{K - m}
        \end{equation*}. &
    ...
    \begin{answercols}
        ...
        \\
        If $X = t$, then tie $\rightarrow$ random guess. &
        Tie-breaking assumption. \\
        \\
        Error if $X > t$ or $(X = t \text{ and guess is wrong})$. &
        There're 2 error-producing senarios. \\
        \\
        \begin{equation*}
            E(K, \epsilon) = \sum_{m = t + 1}^{K} \begin{pmatrix}
                K \\
                m
            \end{pmatrix} \epsilon^{m}(1 - \epsilon)^{K - m} + \frac{1}{2} \begin{pmatrix}
                K \\
                t
            \end{pmatrix} \epsilon^{t}(1 - \epsilon)^{t}
        \end{equation*}. &
        ...
        \\
        \begin{equation*}
            E(K, \epsilon) = \sum_{m = K / 2 + 1}^{K} \begin{pmatrix}
                K \\
                m
            \end{pmatrix} \epsilon^{m}(1 - \epsilon)^{K - m} + \frac{1}{2} \begin{pmatrix}
                K \\
                K / 2
            \end{pmatrix} \epsilon^{K / 2}(1 - \epsilon)^{K / 2}
        \end{equation*}. &
    ...
    \begin{equation*}
        ...
        \begin{cases}
            ...
            \begin{pmatrix}
                ...
                m
            \end{pmatrix}
            \epsilon^{m}(1 - \epsilon)^{K - m}, & K \text{ is odd,} \\\\
            E(K, \epsilon) = \sum_{m = K / 2 + 1}^{K} 
            \begin{pmatrix}
                K \\
                m
            \end{pmatrix}
            \epsilon^{m}(1 - \epsilon)^{K - m} + \frac{1}{2}
            \begin{pmatrix}
                K \\
                K / 2
            \end{pmatrix}
            \epsilon^{K / 2}(1 - \epsilon)^{K / 2}, & K \text{ is even.}
        \end{cases}
    \end{equation*}

    \noindent This should account for independent classifier errors and the majority voting mechanism.
    ...
    Let input be $x \in R^{d}$, hidden layers have arbitary widths, and the output be $y \in R^{m}$.\\

    \noindent\textbf{For the case of One Hidden Layer}
    \begin{answercols}
        Hidden is $h = c(W_{1}x + b_{1})$. &
        Applying linear activation. \\
        \\
        Output is $y = c(W_{2}h + b_{2})$. &
        Doing a second linear transformation. \\
        \\
        $y = c(W_{2}[c(W_{1}x + b_{1})] + b_{2})$. &
        Composition. \\
        \\
        $y = c^{2}W_{2}W_{1}x + c^{2}W_{2}b_{1} + cb_{2}$. &
        Expanidng terms. \\
        \\
    ...
    \begin{answercols}
        $L$ hidden layers, so $y = Wx + b$. &
        This is a claim we're making, will be \textbf{proven via induction.} \\
        \\
        Base case: $L = 0$, $y = Wx + b$. &
        Answered trivially, there are no hidden layers. \\
        \\
        Inductive step: Assume true for $L = k$ layers. & 
        The inductive hypothesis. \\
        \\
        For $L = k + 1$: First $k$ layers are $= h = W_{k}x + b_{k}$. &
        By hypothesis. \\
        \\
        Add layer $k + 1$ such that $y = c(W_{k + 1}h + b_{k + 1})$. &
        Linear activation. \\
        \\
        $y = c(W_{k + 1}(W_{k}x + b_{k}) + b_{k + 1})$. &
        Composition. \\
        \\
        $y = cW_{k + 1}W_{k}x + cW_{k+1}b_{k} + cb_{k + 1}$. &
        Expanding terms. \\
        \\
        This is essentially $y = W'x + b'$. &
    ...
    \begin{answercols}
        ...
        \\
        $\therefore$ Linear networks can only learn from linear $f(x)$. &
    ...
    \begin{itemize}
        ...
        \item Tke the gradient with respect to $\overset{\rightarrow}{\theta}$
        \item Set the gradient to 0 and solve for $\overset{\rightarrow}{\theta}^{*}$
        \item Make sure its a minimum
    \end{itemize}

    \noindent Before solving this though, I will clarify on some variables within the expression: 
    \begin{itemize}
        \item \begin{equation*}
    ...
    \begin{answercols}
        ...
        Simlify by dropping $\frac{1}{N}$. \\
        \\
        $\Phi^{T} R\Phi\theta = \Phi^{T} Ry$. &
        Normal equations. \\
        \\
        Assume that $\Phi^{T} R\Phi$ is invertible. &
        This is the full-rank condition. \\
        \\
        $\theta^{*} = (\Phi^{T} R\Phi)^{-1}\Phi^{T} Ry$. &
        This is the optimal solution. \\
        \\
        Hessian $\nabla_{\theta}^{2}L(\theta) = \frac{1}{N}\Phi^{T}R\Phi \succ 0$. &
        Basically saying that the positive definitive is the minimum.
    \end{answercols}

    \noindent In conclusion, the optimal paramter for the weighted linear regression is the following:
    \begin{equation*}
        \theta^{*} = (\Phi^{T} R\Phi)^{-1}\Phi^{T} Ry
    \end{equation*}
    where $R$ is the diagonal weight matrix, $\Phi$ is the feature matrix, and $y$ is the tgt vector.
    ...
    \noindent\textbf{Note:} this is a \textbf{proof} question, meaning you must follow formal proof structure (see the examples of piazza for guidance). \\
    \\
    This problem can be solved with the probablisitc split method. Since the desicion tree has missing values, we treat missing values as having probablity distribution over possible values, then use frctional examples instead of just throwing away that data.\\
    \\
    ...
    \begin{answercols}
        Input is exmaple $\overset{\rightarrow}{x}$, tree $T$. &
        The exmaple may have missing values. \\
        \\
        Ouput is the predicted class $\overset{\wedge}{y}$. &
        Weighted combination. \\
        \\
        Init. with leaves = $\theta$. &
    ...
    \noindent For the modified information gain calculations, we can suppose at node contruction with dataset $D$, let $D_{\text{complete}} \subset D$ be exmaples with known attribute $A$. 
    \begin{answercols}
        Let $D_{\text{miss}} = D \ D_{\text{complete}}$. &
        All exmaples missing $A$. \\
        \\
        ...
        To handle missing exmaples, and then to split up into all values. \\
        \\
    ...
    \noindent In total, this series of algorithm should handle missing values consistently in both classification and traning. This is because: 
    \begin{itemize}
        \item The probablistic splits are based off of value frequencies during classififcation.
        \item The algorithm employs fractional exmaples based on value distributions during traning.
