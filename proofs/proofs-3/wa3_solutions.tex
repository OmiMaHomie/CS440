...
\begin{document}
    ...
    \\
    Let $D$ be the dataset with $n$ positive examples $(+1)$ and $n$ negative examples $(-1)$ such that $|D| = 2n$. \\
    ...
    \textit{Questions for this question:}
    \begin{itemize}
        \item Should I explicitly prove that $n > n-1$ guarantees no ties, or is this obvious enough for the proof?
        \item To be honest, while I was able to get to this conclusion, I find this conclusion to be kinda paradoxical? Why wouldn't to be a ~50\% accuracy? Is it just that this specific case somehow breaks the algorithm?
    \end{itemize}
...
\\
    ...
    \begin{answercols}
        $X \sim \text{Bin}(K, \epsilon)$. &
        number of wrong classifiers is binomial. \\
        \\
    ...
    \textit{Questions for this question:}
    \begin{itemize}
        \item Should I further clarify the rate at which ensemble error decreases with K, or is deriving the exact formula sufficient?
    \end{itemize}
    ...
    \textit{Questions for this question:}
    \begin{itemize}
        \item For the induction proof in the second part, should I be more explicit about the dimensionality of the weight matrices and how they compose? I showed the linear composition but I dunno if I should then prove how the resulting matrix has the correct dimensions.
        \item The last part of my answer shows how at least one nonlinear unit is necessary for nonlinear relationships. Is how I explained it sufficent?
        \item For the arbitary layers case, should I specify that all layers have the same constant $c$, or would the result hold even if each layer had a different linear constant $c_{i}$?
    \end{itemize}
